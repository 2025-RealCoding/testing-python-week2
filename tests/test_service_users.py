...
def test_authenticate_unregistered_user(setup_user_db):
    # Given : 등록되지 않은 사용자의 얼굴 이미지가 주어짐
    ...
    # Then : 인증 기능이 실패하여 None을 반환
    assert user_id is None


def test_get_registered_users(setup_user_db):
    # Given : 이미 등록된 사용자 ID
    user_id = "aaron_peirsol"
    ...
    # Then : 등록 시간 등 사용자 정보를 정확히 반환해야 함
...
def test_get_unregistered_user():
    ...
    # Then : 조회 결과는 None 이어야 함 (또는 예외가 발생할 수도 있음)
    assert user_info is None
