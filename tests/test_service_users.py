...
def test_authenticate_deleted_user():
    img_path = "images/Aaron_Peirsol/Aaron_Peirsol_0001.jpg"
    with open(img_path, "rb") as f:
        register_user("aaron_peirsol", f.read())

    assert delete_user("aaron_peirsol") is True

    with open(img_path, "rb") as f:
        detected_id = authenticate_user(f.read())

    assert detected_id is None
    assert get_user("aaron_peirsol") is None
